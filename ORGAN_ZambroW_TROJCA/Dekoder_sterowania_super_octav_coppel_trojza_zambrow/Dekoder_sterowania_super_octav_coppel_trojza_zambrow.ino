...
void MyHandleNoteOff(byte channel, byte pitch, byte velocity) {
  if (channel == decoder_channel) {  
    if (pitch = 42) digitalWrite (pins[0], LOW);
    if (pitch = 43) digitalWrite (pins[1], LOW);
    if (pitch = 53) digitalWrite (pins[2], LOW);
    if (pitch = 54) digitalWrite (pins[3], LOW);
    if (pitch = 55) digitalWrite (pins[4], LOW);
